...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        network = build_mlp(input_size=self.observation_dim, output_size=self.action_dim, n_layers=self.config.n_layers,
                            size=self.config.layer_size).to(device)
        if self.discrete:
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            for i in range(len(rewards)):
                ...
                gamma_pwr = np.power(self.config.gamma, exp)
